...
class MyWindow(QMainWindow):
    ...
    #         self.full_chapter_path = os.path.join(self.vocab_source_path, self.current_section, f"Section_{i+1}")
    #         self.current_chapter = f"Section_{i+1}"
    #         print("-------------------------------------------------------------------------------------------------------------------\n")
    #         print( f"Section_{i+1}")
    #         for word in words:
    ...
    def Question_Vocab_Search_Button_clicked(self, insert=False):
        ...
        if len(definition) == 0:
            content = [search_word, "", "", ""]
            print("cannot find definition of word: ",search_word)
            return -1
        output_str = ''
        defiList = []
        self.content_list = []
