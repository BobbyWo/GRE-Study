...
class MatchingGameWindowFullChapter(QMainWindow):
    ...
    def confirmButtonClicked(self):
        ...
        while(len(shuffled_words)>0):
            ...
            page_container = self.setup_vocab(random_words,self.meaning_list,self.example_list,index_list,page_index)
    ...
    def setup_vocab(self,words_list,meaning_list,example_list,index_list,page_index):
        ...
        #set up example
        example_layout = self.setup_example(example_list,index_list)
        one_page_layout.addStretch(1)
        one_page_layout.addLayout(example_layout)
    ...
    def setup_example(self,example_list,index_list):
        # set up answer
        ...
        for index, word_index in enumerate(index_list):
            ...
            # item.posChanged.connect(self.checkCoverage)
            item.setWordWrap(True)
            item.setStyleSheet("border: 1px solid black;border-radius: 10px;padding-top: 10px;padding-left: 10px;padding-right: 10px")
            item.setFixedSize(200, 200)
            item.setAlignment(Qt.AlignmentFlag.AlignCenter)
            # item.set_data(word_index)
            # self.answer_list.append(item)
            example_layout.addWidget(item)
